...
(() => {
  ...
  var DataScript = {
    ...
    title: {
      field: "title",
      label: "T\xEDtulo do Widget",
      description: "aparecer\xE1 no cabe\xE7alho do Widget",
    ...
    open: {
      ...
      description: "ao carregar a p\xE1gina abrir\xE1 o widget automaticamente",
      placeholder: "",
      default: "true"
    },
    screenshot: {
      field: "screenshot",
      label: "Ativar o bot\xE3o Screenshot?",
      description: "Bot\xE3o para o usu\xE1rio tirar um screenshot da tela",
      placeholder: "",
      default: "true"
    },
    apiFaq: {
  ...
  // src/components/ScriptWidget/screenshot.ts
  var import_html2canvas = __toESM(require_html2canvas());
  async function widgetScreenshot() {
    const doc = document.body;
    const canvas = await (0, import_html2canvas.default)(doc);
    return canvas.toDataURL("images/png");
  }

  // src/components/ScriptWidget/index.ts
  var iframeWidget = "iframeWidget";
  (function() {
    ...
    let srcIframe = src;
    srcIframe = src.slice(0, src.lastIndexOf("/"));
    srcIframe += "/widget?";
    Object.entries(DataScript).map(([key, item]) => {
      srcIframe += `${key}=${doc.getAttribute(`data-${key}`) ? encodeURIComponent(doc.getAttribute(`data-${key}`) ?? "") : item.default}&`;
    });
  ...
  var isIFrame = (input) => input !== null && input.tagName === "IFRAME";
  async function OnMessage(e) {
    let width = e.data.width;
    let height = e.data.height;
    const iFrameElement = document.getElementById(iframeWidget);
    if (!iFrameElement) {
      console.log("element widget n\xE3o foi localizado na p\xE1gina!");
      return false;
    }
